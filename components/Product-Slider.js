...
import { Button, Card, CardActionArea, CardActions, CardContent, CardMedia, Container, FormControl, InputLabel, MenuItem, Select, Typography, useMediaQuery } from "@mui/material";
  ...
  return (
    <Box sx={{ width: "100%", position: "relative", maxWidth: "1352px", margin: "auto", overflow: "hidden" }} >
      <Box sx={{ position: 'absolute', right: "-130px", bottom: "0px" }} >
        <Image src={bg} style={{ transform: "rotate(180deg)" }} width="250px" height="300px" />
      </Box>
      <Box m="auto" maxWidth="1052px" position={'inherit'} >
        <Box sx={{ width: "100%", display: "flex", flexDirection: "row", gap: "30px", alignItems: "center" }} >

          <FormControl sx={{ m: 1, minWidth: 140, border: "none", color: "black", boxShadow: "none", fontWeight: 400, textAlign: "center" }} size="small">
            <Select
              variant="outlined"
              labelId="Category"
              id="Category"
              label="All"
              sx={{
                backgroundColor: "#1BFBB5", border: "none", color: "black", boxShadow: "none", outline: "none", textAlign: "center",
                '.MuiOutlinedInput-notchedOutline': { border: 0 }, fontWeight: 400
              }}
              value={value}
          ...
          <Typography variant='text' component='h3' sx={{ cursor: "pointer", fontSize: "16px" }} fontWeight={400}>
            Latest
          </Typography>
          <Typography variant='text' component='h3' sx={{ cursor: "pointer", fontSize: "16px" }} fontWeight={400}>
            Featured
                  ...
                  <Card sx={{
                    ...
                    <CardActionArea sx={{ "&:hover": { backgroundColor: 'transparent', } }} >
                      <CardMedia sx={{
                        ...
                        justifyContent: 'center', width: '230px', height: '250px',
                      ...
                      <CardContent sx={{ textAlign: "left", mb: 0 }} >
                        <Typography variant="body2">
                          {product.category} <span style={{ fontWeight: '500', fontSize: '11px', padding: "5px 5px", marginLeft: '5px', borderRadius: "5px", backgroundColor: state?.config?.primaryColor || '#1BFBB5' }}>20% OFF</span>
                        </Typography>
                        <Typography variant="body1" component="h2" fontWeight={600} mt='5px' mb='10px'>
                          {product.title.slice(0, 25)}
                        </Typography>
                        <Typography variant="body2" sx={{ fontWeight: '600', fontSize: '15px', color: state?.config?.primaryColor || '#0A6BF8' }} >
                          {product.price}
                        </Typography>
                      </CardContent>
                      ...
                      <CardActions sx={{
                        display: 'flex', alignItems: 'center',
                        justifyContent: 'space-between', mb: '10px', py: 0,
                        width: "100%"
                      }}>

                        {hasWindow && QuantityCount(state, product.id) === 1 &&
                          <Box sx={{ cursor: 'pointer' }} onClick={() => removeHandler(product)} >
                            <DeleteIcon sx={{
                              fontSize: '35px',
                              color: '#1BFBB5',
                              "&:hover": { color: '#01579b' }
                        ...
                        {hasWindow && QuantityCount(state, product.id) > 1 &&
                          <Box sx={{ cursor: 'pointer' }} onClick={() => minusHandler(product)} >
                        ...
                        {hasWindow && IsInCart(state, product.id) ?
                          ...
                          <Box sx={{ cursor: 'pointer' }} onClick={() => plusHandler(product)}>
                            <AddBoxIcon sx={{
                              fontSize: '35px',
                              color: '#1BFBB5',
                              "&:hover": { color: '#01579b' }
                          ...
                          <Button variant="contained" sx={{ width: "100%", backgroundColor: "#1BFBB5" }}
                            color="info" onClick={() => addHandler(product)} >
                            Add to cart
                          </Button>}
